...
df = pd.read_csv(r'c:\Users\Diego\OneDrive\Importex\Sales-Manager-Importex\files\IMPORTEX - PEDIDOS 2026  - MARZO.csv', encoding='utf-8-sig', dtype=str)
...
print(f"FilmPro row count in MARZO.csv: {len(fp)}")
print("\n--- Distribution by Month in MARZO.csv (FilmPro) ---")
print(fp['parsed_date'].dt.month.value_counts())

print("\n--- Returns (CST DEV > 0) in MARZO.csv by Month ---")
...
if not rets.empty:
    ...
    print("No returns for FilmPro in MARZO.csv")
